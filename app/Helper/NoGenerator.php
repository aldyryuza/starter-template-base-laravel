...
{
    $data_status_verifikasi = ['ON PROCESS', 'VERIFIED', 'DONE', 'COMPLETE', 'FINISHED'];
    $data = [];
    if ($status == 'VERIFIED') {
...
function ScanPDF($file)
{
    ...
    $namaPattern        = '/Nama\s*:\s*([^\n\r]+)/';   // hanya ambil satu baris
    $perjanjianPattern  = '/Nomor\s+(\d{10,})/';       // tangkap 6602405558

    if (preg_match($nomorPattern, $text, $nomorMatches)) {
        $currentData['nomor'] = trim($nomorMatches[1]);
    }
    if (preg_match($tahunPattern, $text, $tahunMatches)) {
        $currentData['tahun'] = trim($tahunMatches[1]);
    }
    if (preg_match($tanggalPattern, $text, $tanggalMatches)) {
        $currentData['tanggal'] = trim($tanggalMatches[1]);
    }
    if (preg_match($jamPattern, $text, $jamMatches)) {
        $currentData['jam'] = trim($jamMatches[1]);
    }
    if (preg_match($namaPattern, $text, $namaMatches)) {
        $currentData['nama_pemberi'] = trim($namaMatches[1]);
    }
    if (preg_match($perjanjianPattern, $text, $perjanjianMatches)) {
        $currentData['nomor_perjanjian'] = trim($perjanjianMatches[1]);
    }
...
function getLeveCompanyUser($kode_branch)
{
    $result = '';
    $data = Branch::where('kode_cabang', $kode_branch)->first();
    // dd($data);
    if ($data) {
        $data->primary_branch == 1 ? $result = 'is_pusat' : $result = 'is_cabang';
    }
    return $result;
}


function cekDuplikasiNoKontrak($data_no_contract)
{
    $data = [];
    $data = RequestCertificateContract::whereIn('contract_number', $data_no_contract)
        ->whereNull('deleted')
        ->whereIn('status', ['DRAFT', 'ON PROCESS'])
        ->get()->toArray();
    return $data;
}
